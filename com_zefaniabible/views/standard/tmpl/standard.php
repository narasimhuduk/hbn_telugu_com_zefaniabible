...
class StandardView {
	...
	{	
		...
		$document->addStyleSheet(JUri::base() . 'components/com_zefaniabible/css/bar-ui.css');
		$document->addScript(JUri::base() . 'components/com_zefaniabible/js/soundmanager2.js');
		$document->addScript(JUri::base() . 'components/com_zefaniabible/js/bar-ui.js');
	//	$document->addScript( 'https://code.jquery.com/mobile/1.5.0-alpha.1/jquery.mobile-1.5.0-alpha.1.min.js');
		

	?>
	
	<div class="sticky-wrapper"><div id="0" class="textAudio ym-noprint">
...
<div class="sm2-playlist-target"><ul class="sm2-playlist-bd"><li>

<?php echo JText::_('ZEFANIABIBLE_BIBLE_BOOK_NAME_'.$item->int_Bible_Book_ID); ?> <?php echo mb_strtolower(JText::_('ZEFANIABIBLE_BIBLE_CHAPTER'),'UTF-8')."-".$item->int_Bible_Chapter; ?>
...
<li class="selected"><a href="<?php echo JUri::base().'media/com_zefaniabible/audio/telugu-audio-bible/'.$item->int_Bible_Book_ID; ?>-<?php echo $item->int_Bible_Chapter; ?>.mp3" target="_blank">
<?php echo JText::_('ZEFANIABIBLE_BIBLE_BOOK_NAME_'.$item->int_Bible_Book_ID); ?> <?php echo mb_strtolower(JText::_('ZEFANIABIBLE_BIBLE_CHAPTER'),'UTF-8')."-".$item->int_Bible_Chapter; ?></a></li>
</ul>
</div>
</div>
</div>
        ...
        <form action="<?php echo JFactory::getURI()->toString(); ?>" method="post" id="adminForm" name="adminForm">
            <div id="zef_Bible_Main<?php if($item->str_tmpl == "component"){?>_tmpl_comp<?php }?>">
                <div class="zef_legend">
                    ...
                    if(($item->flg_reading_rss_button)and($item->str_tmpl != "component")){?>
                    ...
                    if(($item->flg_email_button)and($item->str_tmpl != "component")){?>
                    ...
                    <?php } ?>
                    <marquee class="cust_marquee">ఈ క్రింది బైబిల్ కు క్రాస్ రెఫెరెన్సులు, స్ట్రాంగ్స్ నంబర్స్ జత చేస్తూ ఉన్నాము. ఆ ప్రక్రియ పూర్తవ్వడానికి ఇంకొంత కాలం పడుతుంది. ఈ ప్రక్రియ అతి త్వరలో పూర్తయ్యి, విశ్వాసులకు పూర్తిస్థాయిలో అందుబాటులోకి వచ్చులాగున దయచేసి ప్రార్ధించండి.</marquee>
                    <?php
                    	$book_name = trim(JText::_('ZEFANIABIBLE_BIBLE_BOOK_NAME_'.$item->int_Bible_Book_ID));
	$book_name_url = str_replace(" ","-",trim(JText::_('ZEFANIABIBLE_BIBLE_BOOK_NAME_'.$item->int_Bible_Book_ID)));
	$selected_book_url = JURI::root()."bible/standard/telugu/".$item->int_Bible_Book_ID."-".$book_name_url."/";
	?>
                    
                    <div class="custom_show_books_heading"  style="display: none;">
                        <input type="hidden" name="selected_bible_book" id="selected_bible_book" value="<?php echo $item->int_Bible_Book_ID; ?>" />
                         <input type="hidden" name="selected_bible_book_link" id="selected_bible_book_link" value="<?php echo $selected_book_url; ?>" />
                        <input type="hidden" name="selected_bible_chapter" id="selected_bible_chapter" value="<?php echo $item->int_Bible_Chapter; ?>" />
    <div onclick="slidedownBooks()">Show Books</div></div>
<div class="custom_hide_books_heading" >
    <div onclick="slideupBooks()">Hide Books</div></div>
    
  <div class="bible-books-cnt-main custom_bible_books_and_verses_cnt" >
      
      

    

<div class="bible-books-cnt ">
 <div class="old-and-new-tabs">
<ul>
<li class="selected old-test-tab testment-tabs-0" onclick="selectBookTabsText(0)"><?php echo JText::_('ZEFANIABIBLE_BIBLE_OLD_TEST');?></li>
<li class="new-test-tab testment-tabs-1" onclick="selectBookTabsText(1)"><?php echo JText::_('ZEFANIABIBLE_BIBLE_NEW_TEST');?></li>
</ul>
</div>
<div class="all-books-cnt testment-cnt-0">
	<ul>
	<?php for($books = 1; $books <= 39; $books++) { 
	$book_name = trim(JText::_('ZEFANIABIBLE_BIBLE_BOOK_NAME_'.$books));
	$book_name_url = str_replace(" ","-",trim(JText::_('ZEFANIABIBLE_BIBLE_BOOK_NAME_'.$books)));
	?>
			<li class="book_row_no_<?php echo $books;?>" onclick="showChapters(<?php echo $books;?>, '<?php echo JURI::root()."bible/standard/telugu/".$books."-".$book_name_url;?>/')"><?php echo $book_name;?></li>
	<?php } ?>	
	</ul>
</div>
 
<div class="all-books-cnt testment-cnt-1" style="display:none;">
	<ul>
	<?php for($books = 40; $books <= 66; $books++) { 
	$book_name = trim(JText::_('ZEFANIABIBLE_BIBLE_BOOK_NAME_'.$books));
	$book_name_url = str_replace(" ","-",trim(JText::_('ZEFANIABIBLE_BIBLE_BOOK_NAME_'.$books)));
	?>
			<li class="book_row_no_<?php echo $books;?>" onclick="showChapters(<?php echo $books;?>, '<?php echo JURI::root()."bible/standard/telugu/".$books."-".$book_name_url;?>/')"><?php echo $book_name;?></li>
...
<div class="bible-books-chapters">
	<?php for($chapters = 1; $chapters <= 50; $chapters++) { 
	$chapter_link = JURI::root()."bible/standard/telugu/1-".JText::_('ZEFANIABIBLE_BIBLE_BOOK_NAME_1')."/".$chapters;;
	?>
	<div class="chapter_row_no_1_<?php echo $chapters;?>"><a href="<?php echo $chapter_link;?>"><?php echo $chapters;?></a></div>
	<?php } ?>	 
</div>

</div>
                  <div style="clear:both;"></div>  
                    <div class="zef_bible_Header_Label page-header">
                        <h1 class="zef_bible_Header_Label_h1"><?php echo JText::_('ZEFANIABIBLE_BIBLE_BOOK_NAME_'.$item->int_Bible_Book_ID); ?></h1>
                        
                         <h2 class="zef_bible_Header_Label_h2"><?php echo mb_strtolower(JText::_('ZEFANIABIBLE_BIBLE_CHAPTER'),'UTF-8')."-".$item->int_Bible_Chapter; ?></h2>
                            ...
                            {																	
                                foreach($item->arr_references as $obj_references)
                                {
                                    if($obj_references->verse_id == $arr_verse->verse_id)
                                    {
                                        ...
                                        echo '<div class="zef_reference_hash">'.$str_pre_link.JText::_('[TSK]').'</a></div>';	
                                        break;
                                    }
                                }
                            }		
