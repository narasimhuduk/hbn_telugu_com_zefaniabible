...
class ZefaniabibleCommonHelper
{
	...
	{	
	    ...
	    $translation_lang = "";
		if($item->str_Bible_Version != "telugu") {
		    $translation_lang = "_EN";
		}
...
/*		if(($item->flg_show_dictionary)and(count($this->arr_dictionary_list) > 1))
		...
		if($item->int_Bible_Chapter > 1)
		{
			...
			if($item->flg_show_pagination_type == 0)
			{
				echo '<input title="'.JText::_('ZEFANIABIBLE_BIBLE_LAST_CHAPTER').'" type="button" id="zef_Buttons" class="zef_lastChapter" name="lastChapter" onclick="'.$urlPrepend.$url[1].$urlPostpend.'"  value="'. JText::_("ZEFANIABIBLE_BIBLE_BOOK_NAME_".$item->int_Bible_Book_ID.$translation_lang)." ".($item->int_Bible_Chapter-1).'" />';
			}
			else
			{
				echo "<a title='".JText::_('ZEFANIABIBLE_BIBLE_LAST_CHAPTER')."' id='zef_links' href='".$url[1]."'>".JText::_("ZEFANIABIBLE_BIBLE_BOOK_NAME_".$item->int_Bible_Book_ID.$translation_lang).' '.($item->int_Bible_Chapter-1)."</a> ";
			}
		}
		if($item->int_Bible_Chapter < $item->int_max_chapter)
		{
			...
			if($item->flg_show_pagination_type == 0)
			{			
				echo '<input title="'.JText::_('ZEFANIABIBLE_BIBLE_NEXT_CHAPTER').'" type="button" id="zef_Buttons" class="zef_NextChapter" name="nextChapter" onclick="'.$urlPrepend.$url[0].$urlPostpend.'"  value="'. JText::_('ZEFANIABIBLE_BIBLE_BOOK_NAME_'.$item->int_Bible_Book_ID.$translation_lang)." ".($item->int_Bible_Chapter+1).'" />';
			}
			else
			{
				echo "<a title='".JText::_('ZEFANIABIBLE_BIBLE_NEXT_CHAPTER')."' id='zef_links' href='".$url[0]."'>".JText::_('ZEFANIABIBLE_BIBLE_BOOK_NAME_'.$item->int_Bible_Book_ID.$translation_lang).' '.($item->int_Bible_Chapter+1)."</a> ";
			}
		}
		if($item->int_Bible_Book_ID < 66)
		{
			...
			if($item->flg_show_pagination_type == 0)
			{			
				echo '<input title="'.JText::_('ZEFANIABIBLE_BIBLE_NEXT_BOOK').'" type="button" id="zef_Buttons" class="zef_NextBook" name="nextBook" onclick="'.$urlPrepend.$url[2].$urlPostpend.'"  value="'. JText::_('ZEFANIABIBLE_BIBLE_BOOK_NAME_'.($item->int_Bible_Book_ID+1).$translation_lang).' 1"'.' />';
			}
			else
			{
				echo "<a title='".JText::_('ZEFANIABIBLE_BIBLE_NEXT_BOOK')."' id='zef_links' href='".$url[2]."'>". JText::_('ZEFANIABIBLE_BIBLE_BOOK_NAME_'.($item->int_Bible_Book_ID+1).$translation_lang).' 1'."</a>";
			} 
		}
	}
	...
	public function fnc_bible_book_dropdown($item)
	{
		...
		$obj_Book_Dropdown = '';
		$obj_Book_Dropdown .= '<optgroup id="oldTest" label="'.JText::_('ZEFANIABIBLE_BIBLE_OLD_TEST'.$translation_lang).'">'.PHP_EOL;
		
		for($x = 1; $x <=66; $x++)
		{
			if($item->int_Bible_Book_ID == $x)
			{
				$obj_Book_Dropdown .= '		<option value="'.$x."-".strtolower(str_replace(" ","-",JText::_('ZEFANIABIBLE_BIBLE_BOOK_NAME_'.$x.$translation_lang))).'" selected>'.JText::_('ZEFANIABIBLE_BIBLE_BOOK_NAME_'.$x.$translation_lang).'</option>'.PHP_EOL;						
			}
			else
			{
				$obj_Book_Dropdown .= '		<option value="'.$x."-".strtolower(str_replace(" ","-",JText::_('ZEFANIABIBLE_BIBLE_BOOK_NAME_'.$x.$translation_lang))).'" >'.JText::_('ZEFANIABIBLE_BIBLE_BOOK_NAME_'.$x.$translation_lang).'</option>'.PHP_EOL;				
			}
			if($x == 39)
			{
				$obj_Book_Dropdown .= '</optgroup>'.PHP_EOL.'<optgroup id="newTest" label="'.JText::_('ZEFANIABIBLE_BIBLE_NEW_TEST'.$translation_lang).'">'.PHP_EOL;
			}
		}
	...
	public function custom_get_no_of_chapters($book)
	{
	    	$db = JFactory::getDbo();
	    	
	}
	
	public function custom_get_no_of_verses($book, $chapter)
	{
